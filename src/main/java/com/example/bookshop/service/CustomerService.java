...
public class CustomerService {
    ...
    public void saveCustomerOrderItems(Customer customer){
       ...
       Order order = customer1.getOrders().get(0);
        ...
        int quantity = cartService.getCartItems().stream().map(CartItem::getQuantity).mapToInt(Integer::intValue).sum();
...
//card item -> book > order item
    public Book toBook(CartItem cartItem){
        BookId bookId = new BookId();
        bookId.setId(cartItem.getId());
        bookId.setIsbn(cartItem.getIsbn());
        return  bookDao.findById(bookId).orElseThrow(EntityExistsException::new);
    }
}
